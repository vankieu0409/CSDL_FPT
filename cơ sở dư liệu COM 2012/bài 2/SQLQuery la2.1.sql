create database QLBHL21 --CSDL Lab 2
go
use QLBHL21
go
...
--xóa cột trong bảng( xóa cấu trúc)
alter table khachhang
drop column email
--thêm ràng buộc check
alter table taikhoan
add constraint CK_TAIKHOAN_sotien check(sotien>=1000)
--thêm ràng buộc khóa chính
alter table Tenbang
add constraint ten_rang_buoc primary tencot
-- thêm ràn gbuoojc khóa ngoại
alter table tên bảng
add constraint ten-rngbuoc foreign key tencot

--thêm ràng buộc duy nhất
alter table KHAchhang
add constraint UNQ_KHACHHANG_Dienthoai unique (Dienthoai)

--xóa bảng: xóa từ dưới xóa lên
drop table GiaoDich
drop table TaiKhoan
drop table	KhachHang
drop database QLBHL2
