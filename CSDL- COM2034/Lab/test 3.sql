...
RETURNS TABLE
AS
RETURN (SELECT MaSP FROM SANPHAM WHERE TenSP=@TenSP AND GiaBan=@GiaBan AND SoLuong=@SoLuong )
GO

SELECT MaSP FROM dbo.FN_CAU3(N'nồi',1200000,34)
...
AS
BEGIN
DECLARE @k TABLE( @sopn NVARCHAR(5),@MASP NVARCHAR(5)) 
begin try
